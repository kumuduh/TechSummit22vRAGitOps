...
def getCSPipelines(baseurl,access_token,apiVersion):
    headers = {"Authorization":access_token}
    var_uri = f'/codestream/api/pipelines?apiVersion={apiVersion}&page=0'
    var_Obj = {}
    print(f'Get CS pipeline uri {var_uri}')
    var_respose = requests.get(f'{baseurl}{var_uri}',headers = headers, verify=False)
    if var_respose.status_code != 200:
        print(f'Failed to get vRA CS pipeline')
    else:    
...
def handler(context, inputs):
    ...
    cs_pipes = getCSPipelines(baseurl,access_token,apiVersion)

    cs_pipe_out = []
    #Prepare to commit
    dt = datetime.now()
    branch_name = 'vra-CS-pipeline-' + dt.strftime('%Y%m%d-%H-%M-%S')
    # Create new branch from main    
    source = repo.get_branch("main")
    repo.create_git_ref(ref=f"refs/heads/{branch_name}", sha=source.commit.sha)     
    
    #Get CS pipelines
    for cs_pipe in cs_pipes:
        cs_pipe_out.append(cs_pipes[cs_pipe]["name"])
        cs_pipeline_filename = f'{cs_pipes[cs_pipe]["name"]}.json'
        file_path = f'codestream/{cs_pipeline_filename}'
        print(f'codestream pipeline file path in repo {file_path}')
        #Check If ebs already in the repo
        matching_cs_var = next((item for item in repo_cs if (item.name == cs_pipeline_filename)), False)
        if matching_cs_var:
           print(f"adding new version of CS Var file: {file_path}")
           push(repo, file_path, "CS var file Update", json.dumps(cs_pipes[cs_pipe]), branch_name, update=True)
        else:
           print(f"adding new CS Var file {file_path}")
           push(repo, file_path, "CS var file Create", json.dumps(cs_pipes[cs_pipe]), branch_name, update=False)
