...
def getImageProfiles(baseurl,access_token,apiVersion):
    headers = {"Authorization":access_token}
    #uri_filter_bp = urllib.parse.quote(f"name eq '{zone_name}'")
    i_uri = f'/iaas/api/image-profiles?apiVersion={apiVersion}'
    print(f'Get Image profiles uri {i_uri}')
    i_respose = requests.get(f'{baseurl}{i_uri}',headers = headers, verify=False)
    if i_respose.status_code >= 400:
        print(f'Failed to getvRA Image Profiles')
        i_respose.raise_for_status()
    else:    
        i_Obj = i_respose.json()['content'] 
...
def handler(context, inputs):
    ...
    #Get Image profiles
    i_profiles = getImageProfiles(baseurl,access_token,apiVersion)
    i_profiles_name_out = extract_values(i_profiles,'name')
    print(f'Pushing vRA site: {vra_site} Image Profiles {i_profiles_name_out}')

    dt = datetime.now()
    branch_name = 'vra-config-Image-Profiles-' + dt.strftime('%Y%m%d-%H-%M-%S')
    # Create new branch from main    
    source = repo.get_branch("main")
    repo.create_git_ref(ref=f"refs/heads/{branch_name}", sha=source.commit.sha)     

    i_filename = f'{vra_site}_image_profiles.json'
    file_path = f'vRAConfig/{i_filename}'
    print(f'vRA Image Profile file path in repo {file_path}')
    #Check If actions already in the repo
    
    matching_zone = next((item for item in repo_config if (item.name == i_filename)), False)
    if matching_zone:
        print(f"adding new version of vRA image Profile: {file_path}")
        push(repo, file_path, "vRA Image Profiles Commit for {vra_site}", json.dumps(i_profiles), branch_name, update=True)
    else:
        print(f"adding new vRA site {vra_site} cloud zone")
        push(repo, file_path, "vRA Image Profiles Commit for {vra_site}", json.dumps(i_profiles), branch_name, update=False)
    
    

    # Create Pull Request
    if create_mr.lower() == 'true':    
        pr_title = f'vRA Image Profile realease {dt.strftime("%Y:%m:%d")}'
    ...
    out_puts = {
        'vRA_Image_Profiles': i_profiles_name_out
    }
